...
<title>Manage AlbumType List</title>
	...
	<div class="table-responsive">
		<div class="table-wrapper">
			<div class="table-title" style="background-color: black;">
				<div class="row">
					<div class="col-sm-6">
						<h2>Album Types Data</h2>
					</div>
					<div class="col-sm-6">
					<button class="btn btn-primary" onclick="window.location.href='./AddAlbumTypeController?action=ADD'">Add New AlbumType</button>
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover" id= "table-id">
				...
				<tbody>
<c:forEach  items="${listalbumtype}" var="albumtype">
					<tr>
												
						<td>${albumtype.type_id}</td>
						<td>${albumtype.type_name}</td> 
						<td>${albumtype.type_description}</td>

						<td>
						<%-- <%-- <input type="hidden" name="id" value="${alb.album_id}"> --%>
				 		<a href="${pageContext.request.contextPath}/AddAlbumTypeController?action=EDIT&id=${albumtype.type_id}"><i class="material-icons" style="color:#FFC133;">&#xE254;</i></a>
  						<a href="${pageContext.request.contextPath}/AddAlbumTypeController?action=DELETE&id=${albumtype.type_id}"><i class="material-icons" style="color:red;">&#xE872;</i></a>  
...
<%
	...
	if(logAndAuditDAO.save("Retrieve AlbumTypes", "Admin", email)) {
		System.out.println("AlbumType Retrieve event inserted");
	}
%>
</body>
</html>
