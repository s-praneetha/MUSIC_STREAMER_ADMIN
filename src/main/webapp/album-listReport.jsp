...
<script src="http://localhost:8080/Music_Streamer_Admin/resources/script/pagination.js"></script>
		...
		<div class="table-wrapper">
			<div class="table-title" style="background-color: black;">
				<div class="row">
					...
					<div class="col-sm-6">
					<button class="btn btn-primary" onclick="window.location.href='./UploadAlbumController?action=save'">Add New Album</button>
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover" id= "table-id">
				...
				<tbody>
<c:forEach  items="${ablist}" var="alb">
					<tr>
												
						<td>${alb.album_id}</td>
						<td>${alb.album_name}</td> 
						<td>${alb.album_type_id}</td>
						<td><img src="./AlbumRetrieve?album_name=${alb.album_name}" height="100" width="75"></td>
						<td>${alb.album_description}</td>
						<td>${alb.language}</td> 
						<td>
						<%-- <input type="hidden" name="id" value="${alb.album_id}"> --%>
						<a href="${pageContext.request.contextPath}/AlbumViewerController?action=EDIT&id=${alb.album_id}"><i class="material-icons" style="color:#FFC133;">&#xE254;</i></a>
 						<a href="${pageContext.request.contextPath}/AlbumViewerController?action=DELETE&id=${alb.album_id}"><i class="material-icons" style="color:red;">&#xE872;</i></a>
...
<%
	...
	if(logAndAuditDAO.save("Retrieve Albums", "Admin", email)) {
		System.out.println("Album Retrieve event inserted");
	}
%>
</body>
</html>
